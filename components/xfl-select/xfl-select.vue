<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		...
		>
		<!-- 显示框 -->
		<div v-else class="input" :class="{placeholder: selectText === placeholder}" @tap="onUpperClick" >{{selectText}}</div>
		...
		:style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			...
			>
				<div 
					class="item" @tap="onClickItem(index, item.value)"
...
<script>
	...
	export default {
		...
		computed:{
			...
			innerList(){    // 转换列表的数据格式
			// console.log("mmmmList"+this.list)
		...
		methods: {
			...
			onClickItem(index, value){  // 列表项上的点击事件
				...
				// this.switchMgr.close();   // 开始隐藏，因为会延迟隐藏，所以可以写在这里
